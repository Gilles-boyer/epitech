...
///////////////////////////////////////////////////////////////////////////////
/////////////////////////////Form Newsletter///////////////////////////////////
if (isset($_POST['submit_newletter'])) {
    $title = formatValue($_POST['title']);
    $content = formatValue($_POST['content']);
    ...
    if (!checkValue($title)) {
        $_SESSION['display_error'] = $error_table[7];
    } elseif (!checkValue($content)) {
        $_SESSION['display_error'] = $error_table[8];
    } elseif (!checkLen($title, 2, 50)) {
        $_SESSION['display_error'] = $error_table[7];
    } else {
        $prepar_req = "INSERT INTO news(title, content, created_at, id_user)
        VALUES('$title','$content', now(),'$id')";
        //req bdd
        $req = $bdd->exec($prepar_req);
        $_SESSION['display_confirmation'] = "Newsletter created";
    }
